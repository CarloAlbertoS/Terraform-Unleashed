...
variable "max_pods" {
  description = "Maximum number of PODs active on each node in the AKS node pool (110 is the reccomended value with kubenet, 30 with Azure CNI)"
}
...
variable "availability_zones" {
  description = "List of availability zone for the provisioning of the nodes"
}
...
variable "aks_sp_application_id" {
  description = "client id of the service principal used by AKS to interact with Azure resources"
}

variable "aks_sp_pwd_secret" {
  description = "secret of the service principal used by AKS to interact with Azure resource"
}
